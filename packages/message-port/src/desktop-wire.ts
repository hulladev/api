...
function bytes(value: unknown): Uint8Array {
  if (typeof value !== 'string') throw new TypeError('Invalid Desktop IPC binary value')
  return Uint8Array.from(atob(value), (character) => character.charCodeAt(0))
}

export async function encodeDesktopMessage(value: unknown): Promise<string> {
  ...
  async function encode(value: unknown): Promise<Wire> {
    ...
    if (typeof value !== 'object' || value === null)
      throw new TypeError('Desktop IPC values must be JSON-compatible, bytes or FormData')
    if (seen.has(value)) throw new TypeError('Desktop IPC values must not contain cycles')
    seen.add(value)
    try {
      ...
      if (Object.getPrototypeOf(value) !== Object.prototype && Object.getPrototypeOf(value) !== null) {
        throw new TypeError('Desktop IPC objects must be plain records; encode native values with a contract codec')
      }
      const entries: Wire[] = []
...
export function decodeDesktopMessage(text: string): unknown {
  function decode(value: unknown): unknown {
    if (value === null || typeof value === 'string' || typeof value === 'boolean') return value
    if (typeof value === 'number' && Number.isFinite(value)) return value
    if (!Array.isArray(value)) throw new TypeError('Invalid Desktop IPC wire value')
    if (value.length === 1 && value[0] === 'undefined') return undefined
    if (value.length !== 2) throw new TypeError('Invalid Desktop IPC wire container')
    if (value[0] === 'bytes') return bytes(value[1])
    const entries: unknown = value[1]
    if (!Array.isArray(entries)) throw new TypeError('Invalid Desktop IPC wire entries')
    ...
    if (value[0] === 'object' || value[0] === 'form-data') {
      ...
      for (const entry of entries) {
        if (!Array.isArray(entry) || entry.length !== 2 || typeof entry[0] !== 'string')
          throw new TypeError('Invalid Desktop IPC wire entry')
        const [name, field] = entry as [string, unknown]
        if (form === undefined) {
          if (Object.hasOwn(record, name)) throw new TypeError('Duplicate Desktop IPC object key')
          ...
          if (
            ...
            throw new TypeError('Invalid Desktop IPC multipart file')
          form.append(
            name,
    ...
    throw new TypeError('Unknown Desktop IPC wire container')
  }
