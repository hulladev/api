...
describe('TanStack Query plugin', () => {
  test('exposes shared keys plus bound query and mutation options', async () => {
    ...
    expectTypeOf(client.users.byId.$key.root).toEqualTypeOf<'users/byId'>()
    expectTypeOf<Parameters<typeof client.users.byId.$key.full>[0]>().toExtend<{
      readonly params: { id: string }
    }>()
    const query = client.users.byId.$tanstack.queryOptions(input)
    expectTypeOf(query.queryKey[0]).toEqualTypeOf<'users/byId'>()
    expectTypeOf(query.queryKey[1]).toExtend<{ readonly params: { id: string } }>()
    expectTypeOf<Awaited<ReturnType<typeof query.queryFn>>['body']>().toEqualTypeOf<{ id: string }>()

    expect(client.health.$key.full()).toEqual(['health'])
    expect(client.users.byId.$key.full(input)).toEqual(['users/byId', input])
    ...
    const boundMutation = client.users.byId.$tanstack.mutationOptions(input)
    const unboundMutation = client.users.byId.$tanstack.mutationOptions()
  ...
  test('requires query input and forwards cancellation through request options', async () => {
    ...
    const client = defineClient(contract, {
      baseUrl: 'https://api.example.com',
      fetch: fetcher,
      plugins: [tanstackQueryPlugin({ namespace: 'query' })],
    }).build()

    // @ts-expect-error Input routes require bound query input.
    expect(() => client.users.byId.$query.queryOptions()).toThrow('requires the route input')
    await client.users.byId.$query.queryOptions({ params: { id: 'user-1' } }).queryFn({
