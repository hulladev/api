...
describe('api', () => {
  test('exports createApi as the API factory', () => {
    expect(createApi()).toHaveProperty('router')
  })
  ...
  test('scopes middleware from the api level without mutating the original api', () => {
    ...
    expectTypeOf(viewer).returns.toEqualTypeOf<{ auth: { userId: 'u1' } }>()
    expect(viewer.$meta.middleware).toStrictEqual({
      router: ['auth'],
      procedure: [],
      selected: ['auth'],
    })
    expect(viewer()).toStrictEqual({ auth: { userId: 'u1' } })
  })
  ...
  test('raw output requires exact unawaited output type', () => {
    const api = createApi({ settings: { output: 'raw' as const } })
  ...
  test('raw output rejects promise returns when schema expects a plain value', async () => {
    const api = createApi({ settings: { output: 'raw' as const } })
