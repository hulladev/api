...
Every PR runs semantic preflight, behavior/type checks, built imports and bundle measurements. The performance workflow runs on pull requests, branch creation, branch pushes without an open PR, version tags, and manual dispatch. It measures two common validated Fetch requests on Node 24 on one Ubuntu runner. Head and base revisions use the same harness in alternating order across three fresh processes; raw samples include package versions, commit SHAs, runtime, CPU, and the harness hash. PR runs update one bot comment with links to both commits and the workflow artifact. Fork PRs retain the report as an artifact and job summary because their token cannot write comments. The complete suite remains available manually via `bun run bench`; it is not run automatically. Shared hosted runners are report-only evidence. Make a regression decision only after reproducing a material change on the same controlled machine, checking uncertainty and an absolute latency/memory budget. Never let an aggregate win conceal failed cleanup or a tail-latency regression. The suite does not claim unmeasured feature parity or universal leadership.
...
The focused harness runs a parameterized JSON GET and a small JSON POST through the real client, Fetch transport, mounted server, and response decoder. This excludes network and database time. Compare revisions locally with `node benchmarks/ci/compare.mjs <head-workspace> <base-workspace> <output-directory>` after building each core package. The base defaults to the PR target commit, the previous commit on master pushes, or master for other branches and tags; manual dispatch accepts a tag or commit. An incompatible pre-v2 base is reported as unavailable rather than given a fabricated percentage. Artifacts are retained for 90 days; paired runs provide comparisons across versions without treating results from unrelated hosts as equivalent.
