...
describe('query plugin mutation helper', () => {
  test('exposes shared key helpers and mutation options on finalized handlers', async () => {
    ...
    const boundByIdOptions = routes.byId.$tanstack.mutationOptions(2)
    ...
    expect(routes.byId.$tanstack.mutationOptions(2)).toStrictEqual({
      mutationKey: ['users/byId', 2],
      mutationFn: expect.any(Function),
    })

    const unboundByIdOptions = routes.byId.$tanstack.mutationOptions()
    ...
    expect(routes.byId.$tanstack.mutationOptions()).toStrictEqual({
      mutationKey: ['users/byId'],
      mutationFn: expect.any(Function),
    })

    expect(routes.all.$tanstack.mutationOptions().mutationFn()).toStrictEqual(users)
    await expect(routes.byId.$tanstack.mutationOptions().mutationFn(2)).resolves.toStrictEqual(users[1])
    await expect(routes.byId.$tanstack.mutationOptions(2).mutationFn()).resolves.toStrictEqual(users[1])
  })
  ...
  test('allows aliasing mutation through query plugin settings', () => {
    ...
    const aliased = aliasedRoutes.byId as typeof aliasedRoutes.byId & {
      $tanstack: {
        mutate: ((input: number) => { mutationKey: readonly [string, number] }) &
          (() => { mutationKey: readonly [string] })
      }
    }

    expect(aliased.$tanstack.mutate(2).mutationKey).toStrictEqual(['users/byId', 2])
  })
})
